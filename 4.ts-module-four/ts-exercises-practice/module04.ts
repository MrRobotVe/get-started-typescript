...
/*  TODO: Convert the sortDescending and sortAscending functions to arrow 
    functions. */
const sortASC: compareFuncType = (a, b) => {
  if (a > b) {
    return 1;
  } else if (b > a) {
    return -1;
...
const sortDES: compareFuncType = (a, b) => {
  if (a > b) {
    return 1;
  } else if (b > a) {
    return -1;
...
let myArray1 = buildArray(12, "ASC");
let myArray2 = buildArray(8, "DES");
console.log("ASC", myArray1);
console.log("DESC", myArray2);
...
function buildArray(items: number, sortOrder: "ASC" | "DES"): number[] {
  let randomNumbers = [];
  let nextNumber;
